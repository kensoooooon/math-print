...
9/5
    引き続き機能を理解。
        base_num_makerは、初めに定めたbaseをランダムににかけていく方式での計算
            数のランダム性を重視しつつ、指定した値を超えないようにコントロール
            共通の数をもたせることで、公倍数と公約数が単純な値にならないようにしている？
                最大公約数は、共通の素因数の積で、公約数はその約数
                最小公倍数は共通の素因数＋それ以外の積
                共通の素因数がある程度大きいほうが良いという考えのもとやっている感がある。
        公約数と公倍数を求めるメソッドは、いったん動作原理を置いておく
            とりあえずは動いているし、特に元の数に依存している風でもない。
    
    実際に下限値を取り払った場合、どのような値が出るかをチェックすべきっぽい。
        実装の中には、特に100以下だと動作しない部分は見受けられないっぽい。
        単に単純過ぎる値や答えを避けるためのやり方？
"""
